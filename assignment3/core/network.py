"""
Utility functions to build neural networks for PPO.
...
class SlimFC(nn.Module):
    ...
    def __init__(self, in_size, out_size, initializer=None, activation_fn=True, use_bias=True, bias_init=0.0):
...
def build_one_mlp(input_size, output_size, hidden_size=256):
    return nn.Sequential(
        SlimFC(in_size=input_size, out_size=hidden_size, initializer=normc_initializer(1.0), activation_fn=True),
        SlimFC(in_size=hidden_size, out_size=hidden_size, initializer=normc_initializer(1.0), activation_fn=True),
        SlimFC(
            in_size=hidden_size,
